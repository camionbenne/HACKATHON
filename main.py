...
if i_param==0:
    moy_T_IDF_canicule = T_IDF_canicule.mean(dim="time", skipna=True).values - 273.15
    moy_T_melun_canicule = T_Melun_canicule.mean(dim="time", skipna=True).values - 273.15

elif i_param==1:
    moy_T_IDF_canicule = Tx_IDF_canicule.mean(dim="time", skipna=True).values - 273.15
    moy_T_melun_canicule = Tx_Melun_canicule.mean(dim="time", skipna=True).values - 273.15

elif i_param==2:
    moy_T_IDF_canicule = Tn_IDF_canicule.mean(dim="time", skipna=True).values - 273.15
    moy_T_melun_canicule = Tn_Melun_canicule.mean(dim="time", skipna=True).values - 273.15
...
def AFFICHAGE(i_param, i_periode):
    ...
    boundaries = [0,0.5,1,1.5, 2, 2.5, 3,3.5,4,4.5,5,10]
    ...
    mm0 = ax.pcolormesh(
        lons_data, lats_data, moy_T_IDF_canicule,
        vmin=vmin, vmax=vmax,
    ...
    ax.set_title(f"Anomalie de température en période de canicule par rapport à Melun {periode_bis[i_periode]}", fontsize=14)
